...
func (h *AIHandler) HandleBlogContentGeneration(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&req); err != nil {
		ctx.JSON(http.StatusBadRequest, fmt.Errorf("failed to read the generate request: %w", err))
		return
	}
	generatedBlog, err := h.AIUseCase.GenerateBlogContent(requestCtx, req.Keywords)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, fmt.Errorf("failed to generate blog content: %w", err))
		return
	}
